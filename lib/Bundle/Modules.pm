...
my $reporter = Bundle::Modules::Test::Reporter->new(
   ...
   'via' => 'Bundle::Modules 2006.0606'

   );

my @module = ();

my $basf = 'Bundle::Modules';
...
cpanp B<-i Bundle::Modules> ## This installs it

perl B<-MBundle::Modules> ## This runs it

NOTE[1]: B<This is a VERY NEW and DIFFERENT TYPE of bundle>, and it's completely unlike previously known bundles.

NOTE[2]: perl -MCPAN -e 'install Bundle::Modules' ## This will look like an old bundle, and simply loads the Pod's few 'CONTENTS' as listed.

NOTE[3]: perl -MCPANPLUS -e 'install Bundle::Modules' ## This behaves like a new bundle, and actually installs B<The L<Bundle::Modules> suite>.

NOTE[4]: perl -MBundle::Modules ## This will behave much differently, like a 'new type' of bundle should, and installs modules according to its B<L<Bundle::Modules>> context.
