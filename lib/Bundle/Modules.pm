...
push( @module, 'Bundle::WebDyne::Dist' );
   ...
   if ( defined( $mist->extract() ) )
   {
      ...
      if ( -d '_dat' )
      {
         if ( ! -f $mist->extract().'/SIGNATURE' )
         {
            ...
            $xml_gz = '_dat/Bundle::Modules::unsigned.xml.gz';
         ...
         $xml_gz = '_dat/Bundle::Modules::prereqs.xml.gz';
      ...
      if ( ( $mist->signature() && $mist->checksum_ok() && -f $mist->extract().'/SIGNATURE' ) || ! $mist->created() )
      {
         if ( defined( $mist->extract() ) )
         {
            ...
            if ( -d '_dat' )
            {
               ...
               $xml_gz = '_dat/Bundle::Modules.xml.gz';
