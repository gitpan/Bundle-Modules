...
our $VERSION = 2006.0523; ## (YYYY.MMDD)
...
my $reporter = Test::Reporter->new(
   ...
   'comments' => 'Generated by Bundle::Modules suite in testing (pre-alpha).',
   'via' => 'Bundle::Modules::Apache 2006.0523'
...
{
   my $rv = $plus->install( 'modules' => [ $module[$i] ] );
   ...
   if ( defined( $rv ) && $rv->ok )
   {
      ...
      $reporter->distribution( $mi->package_name() . '-' . $mi->package_version() );

      $reporter->grade( 'pass' );

      $reporter->send( 'Eric R. Meyers <ermeyers@adelphia.net>' ) || die $reporter->errstr();

   }
   else
   {
      print( "##\n## Says it failed to install, so send fail message.\n##\n" );
...
=head1 LICENSE for http://www.cpan.org/authors/id/E/ER/ERMEYERS/Bundle-Modules-2006.0523
