...
my $reporter = Bundle::Modules::Test::Reporter->new(
   ...
   'via' => 'Bundle::Modules::Unstable 2006.0604'
...
for ( my $i = 0; $i <= $#module; $i++ )
{
   ...
   if ( defined( $mist->extract() ) )
   {
      ...
      if ( -d '_dat' )
      {
         if ( ! -f $mist->extract().'/SIGNATURE' )
         {
            ...
            $xml_gz = '_dat/Bundle::Modules::Unstable::unsigned.xml.gz';
         ...
         $xml_gz = '_dat/Bundle::Modules::Unstable::prereqs.xml.gz';
   ...
   if ( ( $context eq 'stable' ) && ! $mist->installed() )
   {
      ...
      if ( -d '_dat' )
      {
         ...
         $xml_gz = '_dat/Bundle::Modules::Unstable::Unstable.xml.gz';
      ...
      if ( ( $mist->signature() && $mist->checksum_ok() && -f $mist->extract().'/SIGNATURE' ) || ! $mist->created() )
      {
         if ( defined( $mist->extract() ) )
         {
            ...
            if ( -d '_dat' )
            {
               ...
               $xml_gz = '_dat/Bundle::Modules::Unstable.xml.gz';
