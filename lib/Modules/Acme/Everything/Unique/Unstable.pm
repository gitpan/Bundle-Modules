package Bundle::Modules::Acme::Everything::Unique::Unstable;

use 5.005;

our $VERSION = 2006.0512; ## (YYYY.MMDD)

1;

__END__

=head1 NAME

=over

Bundle::Modules::Acme::Everything::Unique::Unstable - All current I<unstable> and I<unique> modules in Bundle::Modules::Acme::Everything

=back

=head1 SYNOPSIS

=over

=item C<% B<perl -MCPAN -e 'install Bundle::Modules::Acme::Everything::Unique::Unstable'>>

=item C<% B<cpan -i Bundle::Modules::Acme::Everything::Unique::Unstable>>

=back

=head1 CONTENTS

##Array::Reform
##SQL::Routine
##Apache::WDB
##VMS::ICC
##NexTrieve::Index
##HtDig::Htsearch
##DBA::Backup
##POE::Component::MPG123
##News::Search
##Parse::YALALR
##Carp::CheckArgs
##Log::Check
##Config::Ini
##Business::OnlinePayment::E2Payments
##Apache::Sybase::DBlib
##VMS::Device
##DBD::RDB
##VMS::Priv
##RFID::EPC
##Religion::Package
##Apache::Safe
##Apache::SawAmpersand
##NexTrieve::Querylog
##Astro::SkyCoords
##TeX::Convert::Image
##Date:Jalali
##Sparky
##PDF::Template
##RT
##DBD::Recall
##LockFile::Scheme
##GISI
##Getopt::EvaP
##WWW::Mail::Hotmail
##Tie::RndHash
##Ratpoison
##PDA::Pilot
##SIL::Shoe
##Data::FormValidator::Upload
##Mono
##DCE::RPC
##Fame
##Win32::FileType
##File::sudoers
##HTML::Widgets::DateEntry
##Lingua::EN::Cardinal
##ControlX10::CM10
##Sys::Sysconf
##Lemonolap
##Text::Wrap::Hyphenate
##Solaris::PerlGcc
##Win32::GUID
##Win32::AdminMisc
##Text::Substitute
##Archive::Cabinet
##CORBA::XPIDL
##PostScript::Basic
##Oak
##Apache::MimeDBI
##VMS::Queue
##CGI::SimpleCache
##Filesys::mntent
##ReleaseAction
##GISI::MIFMID
##VMS::smg
##VMS::System
##MIME::IO
##Finance::Shares::MySQL
##MOP
##Apache::Timeit
##Object::Info
##Apache2::PageKit
##SGML::SPGrove
##Tie::PerfectHash
##BizTalk
##Apache::AuthzGender
##IPC::UPDmsg
##Boulder
##Tk::DBIx
##Devel::PerlySense
##PDL::NetCDF
##W3C::LinkChecker
##Class::Flyweight
##Concurrent
##Test::Litmus
##ERG
##UUID
##Tk::ReportBox
##File::Data
##Tk::Autoscroll
##X11::Xforms
##Linksys::Config
##Find::File::Object
##Apache::LowerCaseGETs
##Glade
##NexTrieve::Resource
##Device::SerialPort::Xmodem
##Exporter::PkgAlias
##MMDS
##Net::IPP
##Term::Size::Heuristic
##IPC::Shm::Simple
##NexTrieve::Query
##Genezzo
##HTML::BarChart
##GSM::SMS
##App::Config
##NexTrieve::Document
##Tk::Statusbar
##Lexical::Typeglob
##JUNOS
##NexTrieve::HTML
##Math::Matrix::SVD
##Search::Kinosearch
##Apache::AnonProxy
##Cz::Speak
##CfgTie
##DBD::Google
##Language::VBParser
##Chart::Pie3d
##Tie::Assert
##Finance::Shares::Chart
##Exporter::Import
##Apache::AuthenRaduis
##Apache::Log::Spread
##NexTrieve::DBI
##PDF::PlainLayout
##PostScript::Barcode
##Apache::MsqlProxy
##Interface::Param
##Archive::Parity
##Apache::AdBlocker
##NexTrieve::Docseq
##Image::ASCII
##BoxBackup
##PerlMenu
##Mac::Spotlight
##WebService::BabelFish
##SVN::Look
##Plugin::Language::DML
##Apache::Handler
##CORBA::Python
##Font::TTF
##Data::Dumper::HTML
##NexTrieve::PDF
##Net::USPS
##Mobile::WURFL
##File::ManualFlock
##X11::Toolkit
##AFS
##Games::Go::GoPair
##Getopt::Long::File
##Algorithm::Munkre
##Mail::MH
##CGI::Query
##Apache2::DBI
##HtDig
##Class::Eroot
##Tk::SignOnDialog
##Apache::LogMail
##NexTrieve::Collection::Index
##VMS::Persona
##CGI::Base
##Math::Geometry
##PDF::Report::Table
##Math::FresnalZone
##Apache::MIME
##Fax::DataFax
##Text::Parser
##Math::Nocarry
##Convert::Lucene
##Palm::Magellan::NavCompanion
##CGI::WebApp
##NexTrieve
##Mail::DSN
##HPUX::Ioscan
##Mail::vpopmaild
##ex::caution
##GISI::SHAPE
##AFS::Command
##Regexp::Func
##Authorizenet
##Filesys::dfent
##Lingua::PT
##Apache::NavBar
##CGI::Session::Encrypted
##Lingua::FeatureSet
##Apache::AuthenDBI
##Tk::Multi
##Net::DHCP
##RFID::Matrics
##NexTrieve::Hitlist
##VMS::Process
##Mail::Vacation
##HTML::Formulate
##Python::Err
##Tk::Olwm
##RFID::Base
##Tree::List2Tree
##WWW::Scrobbler::Stats
##Proxy::Tk
##WWW::Search::Magellan
##ePortal
##String::Ruby
##Gemstone
##Thread::Group
##Tie::Plural::Scalar
##Apache::TicketAccess
##Bleach
##Bio::SAGE
##MSSQL::DBlib
##Datascope
##Apache::Checksum
##OpenCA
##Time::JulianDateTime
##BnP
##Net::ICAP
##Lingua::EN::Ordinal
##Geo::Projection
##GSS
##Tk::ImageButton
##Fortran::NameList
##Net::Mac::Vendor
##SOOP
##Tk::FlatCheckbox
##VMS::Fileutils::Root
##SICStus
##PerlPoint::Package
##NexTrieve::RFC822
##NexTrieve::Hitlist::Hit
##Win32::ODBC
##Apache::AuthzAge
##RFID::Alien
##DBD::Altera
##DBD::FreeTDS
##Apache::DayLimit
##NexTrieve::Targz
##Hardware::Simulator
##Apache::ModBT
##Tie::Mem
##Win32::Perfmon
##Attribute::Abstract
##HTML::Info
##Pod::MIF
##Apache::Servlet
##Verilog::GetInfo
##HTML::Stripper
##Apache::AccessLimitNum
##Orac
##Tern::Size::Heuristic
##Win32::FUtils
##WING
##Business::Payroll::AU::PAYG
##Protect
##Win32::IIS:SMTP
##SMIMEUtil
##Openview::Message
##Proc::times
##XML
##Text::Banner
##PDL::IO::HDF5
##Class::PublicInternal
##XMLI
##AI::Categorize
##NexTrieve::Mbox
##Smirch
##Proc::PID::File::Fcntl
##Filesys::statfs
##NexTrieve::Collection
##Apache::PATCH
##Mac::Serial
##RTF::Generator
##PixDraw
##Tk::PhotoRotate
##xDash
##WIN32::MSI::SummaryInfo
##ToolTalk
##Bio::Affymetrix
##Win32::GD
##SQL::Builder
##CGI::FormManager
##Apache::Htaccess
##MSSQL::Sqllib
##Sort::PolySort
##VMS::Monitor
##Tree::Menu
##PDL::LAPACK
##Oracle::OCI
##MARC
##Proc::ExitStatus
##DBIx::TableAdapter
##GD::Barcode::Code93
##WWW::Lyrics
##Business::OnlinePayment::Cybersource
##Logfile
##Devel::Trampoline
##Mail::Maildir
##POE::Session::GladeXML
##Template::TagParser
##SVG::Template
##Ioctl
##FileSys::Tree
##Neural
##Synergy
##Apache::DCELogin
##Text::Wigwam
##MSDOS::SysCalls
##VMS::Fileutils::SafeName
##Tie::WarnGlobal
##Unix::UserAdmin
##VMS::Misc
##Time:Compare
##Finance::Bank::Commonwealth
##Chart::Gdchart
##Watchdog::HTTPService
##File::Size
##Math::BigIntFast
##Text::Stem
##Crypt::Schnorr::AuthSign
##Image::Magick::BarChart
##WWW::TVguide::NL
##Convert::Base
##DBD::SQLrelay
##Apache::AddrMunge
##NexTrieve::Daemon
##EventServer::Signal
##Exporter::Options
##Apache::QuickCMS
##EventServer::Functions
##CGI::FormFactory
##Win32::Pipe
##CGI::Request
##GD::Gauge
##Persistence::Object
##HTML::Demoroniser
##ElectricArc
##IO::Redirect
##Watchdog::Service
##XML::Validator::RelaxNG
##Parse::Lexer
##NetCDF
##Data::Stash
##Class::Holon
##NexTrieve::Overview
##SQL::QueryBroker
##Zeta
##Win32::Message
##CMS::Mediawiki
##MIME::Latin1
##DBIx::XMLMEssage
##Text::Bib
##WWW::Lunarstorm
##Tie::Array::Huge
##PHP::MySQL_Session
##Math::SimpleInterest
##Apache::AuthzDBI
##PDL::Slatec
##WWW::ShopBot
##Apache::BlockAgent
##Apache::StripSession
##HTML::Debug
##Crypt::OpenSSL::BN
##X11::XRT
##Finance::Bank::DE::SpardaBank
##Apache::MimeXML
##Python
##NexTrieve::Search
##NexTrieve::Replay
##CGI::BasePlus
##NexTrieve::Message
##CGI::MiniSvr
##Apache::Command
##Frontier::RPC
##Win32::MSI
##Lemonldap::Portal
##Perl6::Doc
##Apache::AuthzSSL
##PGP
##CGI::DBTables
##NexTrieve::MIME
##CGI::CList
##Apache::SpeedLimit
##Physics::Solar::VSO
##Class::TOM
##Mail::Vacation::LDAP
##FameHLI
##WWW::Search::Excite
##VMS::User
##Business::OnlinePayment::Moneris
##Apache::Forward
##Clips
##Bio::Genex
##AI::NeuralNet
##Getopt::Help
##Chatbot::RiveScript
##Gtk::Dialog
##Apache::AuthAny
##Apache::AuthNISPlus
##Getopt::Gnu
##DBA::Backup::mysql
##Splash::DB
##Math::Interpolate
##Remedy::AR
##File::MultiTail
##Watchdog::MysqlService
##Apache::Upload
##Devel::DebugAPI
##RPM::Headers
##File::Atomic
##Quiz::Question
##Apache::AxKit
##Scalar::Footnote
##Crypt::Camellia
##Slash
##WWW::Webjay
##IO::Filter
##Archive::Tar::Stream
##EventServer::Gettimeofday
##Devel::Truth_Table
##Widget::FixedWidthFont::Input
##Module::MakeDist
##Event::tcp
##NexTrieve::UTF8
##Crypt::ElGamal
##DBIx::SchemaView
##Apache::Magick

=head1 CHANGES

=over

=item 2006.0510 (YYYY.MMDD)

Initial version of: Bundle::Modules::Acme::Everything::Unique::Unstable-2006.0510

This B<L<Bundle::Modules>> component package is in pre-alpha state with hash ordering.

The ordering of listed modules will eventually be prioritized for efficiency and completeness.

Modules commented out of this module list are either I<unstable> or I<stable> in the B<L<Bundle::Modules>> context.

Optimistically, if there are a lot of the modules commented out, then they are I<stable>, and we're listing I<unstable> context.

Optimistically, if there are a few of the modules commented out, then they are I<unstable>, and we're listing I<stable> context.

If you use the command C<B<man Bundle::Modules::Acme::Everything::Unique::Unstable>>, type C<B</#>> to see the commented out modules as they're higlighted for you by B<L<man>>.

=back

=head1 DESCRIPTION

=over

=item This Bundle package will install all of the modules listed, if they're not commented out of the B<L<Bundle::Modules>> context.

=back

=head1 SEE ALSO

=over

=item I<L<Bundle::Modules::Unstable>>, I<L<Bundle::Modules::CPAN>>, I<L<Bundle::Modules::CPAN::Unstable>>, I<L<Bundle::Modules::Acme::Everything>>, I<L<Bundle::Modules::Acme::Everything::Unstable>>, I<L<Bundle::Modules::Acme::Everything::Unique>>, I<L<Bundle::Modules::Acme::Everything::Unique::Unstable>>

=back

=head1 AUTHORS

=over

=item Eric R. Meyers E<lt>ermeyers@adelphia.netE<gt>

=back

=head1 LICENSE for http://www.cpan.org/modules/by-authors/id/E/ER/ERMEYERS/Bundle-Modules-2006.0512

=over

=item http://www.perl.com/perl/misc/Artistic.html

=item This program is free software; you can redistribute it and/or modify it under the same terms as Perl itself.

=back

=head1 COPYRIGHT

=over

=item Copyright 2006 by Eric R. Meyers E<lt>ermeyers@adelphia.netE<gt>

=back


